...
if (!$modx->resource->richtext) {
    return $input;
}
if (!empty($options)) {  // if we're being called as an output filter, set variables for any options
    parse_str($options);
}
...
$phpthumbon = isset($phpthumbon) ? $phpthumbon : '';
...
$remoteDomains = FALSE;
$dom = new DOMDocument;
@$dom->loadHTML('<?xml encoding="UTF-8"><meta charset="utf-8">' . $input);  // load this mother up
...
foreach ($dom->getElementsByTagName('img') as $node) {  // for all our images
  ...
  parse_str($phpthumbon, $opts);  // add in any user-specified phpthumb parameters
  ...
  if (!empty($opts)) {  // have we anything to do for this lovely image?
    ...
    $debug &&   $debugstr .= "phpthumbon options: {$image['options']}\n";
    $node->setAttribute($imgSrc, $modx->runSnippet('phpthumbon', $image));  // do the business and set the src
    if ($updateStyles) {
      $style = '';
...
$output = str_replace('&#13;', '', substr($dom->saveXML($dom->documentElement), 48, -14) );  // strip off the <body> tags and CR characters that DOM adds (?)
$debug &&   $output = "<!--\n$debugstr-->\n$output";
return $output;
return;
