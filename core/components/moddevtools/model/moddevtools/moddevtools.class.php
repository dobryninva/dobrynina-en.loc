...
class modDevTools {
	...
	function __construct(modX &$modx, array $config = array()) {
		...
		$this->config = array_merge(array(
            ...
            'createVirtual' => $this->modx->getOption('moddevtools_virtual_chunks', null, false),
            'debug' => $this->modx->getOption('moddevtools_debug', null, false),
		), $config);
    ...
    public function parseParent($parent, $link_type) {
        if ($parent && $link_type) {
            $link = explode('-', $link_type);
            if ($link[0] == 'temp') {
                $obj = $this->modx->getObject('modTemplate', $parent);
            } else if ($link[0] == 'chunk') {
                $obj = $this->modx->getObject('modChunk', $parent);
            } else {
                return;
            }
    ...
    public function showTemplate() {
        $this->modx->controller->addHtml("<script>
            Ext.onReady(function() {
                ...
                tempCombo.label.update(tempCombo.fieldLabel + '&nbsp;<a target=\"_blank\" href=\"?a=' +
                    (modDevTools.modx23 ? 'element/template/update' : MODx.action['element/template/update']) +
                    '&id=' + tempCombo.value + '\">' + _('open') + '</a>');
            });
            </script>"
        );
    }
