    ...
    */
            ...
            if(in_array(strtoupper($wh[1]),array('IN','NOT IN')) && is_array($val)){
                ...
                foreach($temp_arr as $k => $v){
                    ...
                    if(in_array($wh[1],array('<','>','<=','>=')) && is_numeric($v)){
                        $output['bind'][] = array($wh[0].$count.$cnt, floatval($v), PDO::PARAM_INT);
                    }else{
    ...
    public function getHTMLOutput()
    {
        ...
        foreach($this->products as $product){
            ...
            if ($this->modx->getOption('pdotools_fenom_parser')) {
                $pdoTools = $this->modx->getService('pdoTools');
            }

            if ($this->modx->getOption('pdotools_fenom_parser')) {
                $output .= $pdoTools->getChunk($this->config['tpl'],$properties);
            } elseif( $chunk_name != $this->config['tpl'] ) {
        ...
        if(!empty($this->config['outerTpl'])){
          if ($this->modx->getOption('pdotools_fenom_parser')) {
              $output = $pdoTools->getChunk($this->config['outerTpl'],array('inner' => $output));
          } else {
