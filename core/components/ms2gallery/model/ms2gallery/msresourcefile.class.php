...
class msResourceFile extends xPDOSimpleObject
{
    ...
    public function saveThumbnail($raw_image, $options = array())
    {
        ...
        $this->xpdo->log(modX::LOG_LEVEL_ERROR, '[ms2Gallery] Could not save thumbnail to "' . $resource_file->get('path') . $resource_file->get('file') . '". ' .
            print_r($this->mediaSource->errors, 1));
    ...
    public function updateMS2File()
    {
        ...
        if (!$file = $this->xpdo->getObject('msProductFile', $key)) {
            $file = $this->xpdo->newObject('msProductFile');
            $file->fromArray($key, '', true, true);
            if ($this->parent) {
                if ($parent = $this->getOne('Parent')) {
                    $key['hash'] = $parent->get('hash');
                    if ($product_file = $this->xpdo->getObject('msProductFile', $key)) {
                        $file->set('parent', $product_file->id);
                    }
                }
                if (!$file->get('parent')) {
                    $this->xpdo->log(xPDO::LOG_LEVEL_ERROR, 'Could not sync thumbnail file for "' . $this->get('path') . $this->get('file') . '": parent not found');
