...
class RvgValidator {
    ...
    public $modx = null;
    /**
     * @var Rvg $rvg A reference to the Rvg instance.
     * @access public
     */
    public $rvg = null;
     ...
     *
     * @param Rvg &$rvg A reference to the Rvg class instance.
    ...
    function __construct(Rvg &$rvg,array $config = array()) {
        ...
        $this->config = array_merge(array(
            ...
            'customValidators' => !empty($this->rvg->config['customValidators']) ? explode(',',$this->rvg->config['customValidators']) : array(),
        ),$config);
    }
     ...
     *
     * @access public
     * @param RvgDictionary $dictionary The fields to validate.
     * @param string $validationFields
     * @return array An array of field name => value pairs.
     */
    public function validateFields(RvgDictionary $dictionary,$validationFields = '') {
     ...
     * either be a method name of RvgValidator or a Snippet name.
    ...
    public function validate($key,$value,$type = '') {
        ...
        if (method_exists($this,$type) && !in_array($type,$invNames)) {
            ...
            if ($snippet) {
                ...
                $props = array_merge($this->rvg->config,array(
                    'key' => $key,
                    'value' => $value,
                    'param' => $param,
                    'type' => $type,
                ...
                /* no validator found */
                $this->modx->log(modX::LOG_LEVEL_ERROR,'[Rvg] Could not find validator "'.$type.'" for field "'.$key.'".');
                $validated = true;
            }
        } else {
            $this->modx->log(modX::LOG_LEVEL_INFO,'[Rvg] Validator "'.$type.'" for field "'.$key.'" was not specified in the customValidators property.');
            $validated = true;
        }
    ...
    public function addError($key,$value) {
        $errTpl = $this->modx->getOption('errTpl',$this->rvg->config,'<span class="error">[[+error]]</span>');
    ...
    protected function _getErrorMessage($field,$parameter,$lexiconKey,array $properties = array()) {
        if (!empty($this->rvg->config[$field.'.'.$parameter])) {
            $message = $this->rvg->config[$field.'.'.$parameter];
            $this->modx->lexicon->set($lexiconKey,$message);
            $this->modx->lexicon($lexiconKey,$properties);
        } else if (!empty($this->rvg->config[$parameter])) {
            $message = $this->rvg->config[$parameter];
            $this->modx->lexicon->set($lexiconKey,$message);
            $this->modx->lexicon($lexiconKey,$properties);
        } else {
