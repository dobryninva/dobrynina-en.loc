mvtSeoData.grid.ResourceTemplates = function (config) {
    config = config || {};
    if (!config.id) {
        config.id = 'mvtseodata-resource-templates';
    }
    Ext.applyIf(config, {
        ...
        baseParams: {
            action: 'mgr/templates/resources-getlist'
        },
        listeners: {
    ...
    mvtSeoData.grid.ResourceTemplates.superclass.constructor.call(this, config);
...
Ext.extend(mvtSeoData.grid.ResourceTemplates, MODx.grid.Grid, {
    windows: {},
    ...
    createItem: function (btn, e) {
        var w = MODx.load({
            xtype: 'mvtseodata-resource-template-window-create',
            id: Ext.id(),
            listeners: {
                success: {
                    fn: function () {
        ...
        MODx.Ajax.request({
            ...
            listeners: {
                success: {
                    fn: function (r) {
                        var w = MODx.load({
                            xtype: 'mvtseodata-resource-template-window-update',
                            id: Ext.id(),
                            record: r,
                            listeners: {
                                success: {
    ...
    getFields: function () {
        return ['id', 'name', 'resource', 'classkey', 'pagetitle_template_bl', 'title_template_bl', 'get_page_template_bl', 'description_template_bl', 'content_template_bl', 'active', 'actions'];
    },

    getColumns: function () {
        return [ {
            header: _('mvtseodata_template_resource'),
            dataIndex: 'resource',
            sortable: true,
            width: 150,
        }, {
            header: _('mvtseodata_resource_class_key'),
            dataIndex: 'classkey',
            sortable: true,
            width: 80,
        }, {
...
Ext.reg('mvtseodata-resource-templates', mvtSeoData.grid.ResourceTemplates);
