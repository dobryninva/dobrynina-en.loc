...
if ($transport->xpdo) {
    ...
    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
        ...
        case xPDOTransport::ACTION_UPGRADE:
            $modelPath = $modx->getOption('mvtseodata_core_path', null,
                    $modx->getOption('core_path') . 'components/mvtseodata/') . 'model/';
            $modx->addPackage('mvtseodata', $modelPath);

            $manager = $modx->getManager();
            $objects = array();
            $schemaFile = MODX_CORE_PATH . 'components/mvtseodata/model/schema/mvtseodata.mysql.schema.xml';
