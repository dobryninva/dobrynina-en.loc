...
class msImportExportGalleryUploadProcessor extends msResourceFileUploadProcessor
{
    ...
    public function process()
    {
        ...
        if ($this->modx->getOption('ms2gallery_duplicate_check', null, true, true)) {
            ...
            if ($tmpObject = $this->modx->getObject('msResourceFile', array('resource_id' => $this->resource->id, 'hash' => $data['hash'], 'parent' => 0))) {
                @unlink($data['tmp_name']);
        ...
        $uploaded_file = $this->modx->newObject('msResourceFile', array(
            ...
            'parent' => 0,
            'name' => $this->getProperty('name',preg_replace('#\.' . $extension . '$#i', '', $data['name'])),
            'file' => $filename,
    ...
    {
        if (!$this->modx->getOption('msimportexport.gallery.resize_upload_image', null, 1)) return;
        $tmp = getimagesize($file);
        $properties = $this->mediaSource->getPropertyList();
        if ($tmp[0] > $properties['maxUploadWidth'] || $tmp[1] > $properties['maxUploadHeight']) {
            $options = array();
...
return 'msImportExportGalleryUploadProcessor';
