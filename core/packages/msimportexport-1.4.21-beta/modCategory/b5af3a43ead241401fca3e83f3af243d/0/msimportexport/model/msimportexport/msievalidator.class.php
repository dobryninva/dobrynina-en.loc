...
class MsieValidator {
    ...
    public $modx = null;
    /**
     * @var Msie $msie A reference to the Msie instance.
     * @access public
     */
    public $msie = null;
     ...
     *
     * @param Msie &$msie A reference to the Msie class instance.
     * @param array $config Optional. An array of configuration parameters.
     * @return MsieValidator
     */
    function __construct(Msie &$msie,array $config = array()) {
        ...
        $this->config = array_merge(array(
            ...
            'customValidators' => !empty($this->msie->config['customValidators']) ? explode(',',$this->msie->config['customValidators']) : array(),
        ),$config);
    }
     ...
     *
     * @access public
     * @param MsieDictionary $dictionary The fields to validate.
     * @param string $validationFields
     * @return array An array of field name => value pairs.
     */
    public function validateFields(MsieDictionary $dictionary,$validationFields = '') {
     ...
     * either be a method name of MsieValidator or a Snippet name.
    ...
    public function validate($key,$value,$type = '') {
        ...
        if (method_exists($this,$type) && !in_array($type,$invNames)) {
            ...
            if ($snippet) {
                ...
                $props = array_merge($this->msie->config,array(
                    'key' => $key,
                    'value' => $value,
                    'param' => $param,
                    'type' => $type,
                ...
                /* no validator found */
                $this->modx->log(modX::LOG_LEVEL_ERROR,'[Msie] Could not find validator "'.$type.'" for field "'.$key.'".');
                $validated = true;
            }
        } else {
            $this->modx->log(modX::LOG_LEVEL_INFO,'[Msie] Validator "'.$type.'" for field "'.$key.'" was not specified in the customValidators property.');
            $validated = true;
        }
    ...
    public function addError($key,$value) {
        $errTpl = $this->modx->getOption('errTpl',$this->msie->config,'<span class="error">[[+error]]</span>');
    ...
    protected function _getErrorMessage($field,$parameter,$lexiconKey,array $properties = array()) {
        if (!empty($this->msie->config[$field.'.'.$parameter])) {
            $message = $this->msie->config[$field.'.'.$parameter];
            $this->modx->lexicon->set($lexiconKey,$message);
            $this->modx->lexicon($lexiconKey,$properties);
        } else if (!empty($this->msie->config[$parameter])) {
            $message = $this->msie->config[$parameter];
            $this->modx->lexicon->set($lexiconKey,$message);
            $this->modx->lexicon($lexiconKey,$properties);
        } else {
