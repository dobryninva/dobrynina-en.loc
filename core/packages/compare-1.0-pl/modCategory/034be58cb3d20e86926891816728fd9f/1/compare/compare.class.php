...
function __construct(&$modx, $config = array()){
    ...
    $this->config = array_merge(array(
        ...
        'toCompare_tpl' => '@FILE toCompare.tpl',
...
function toCompareContent(){
    ...
    $chunkArr = array(
        ...
        'href_cancel' => $this->modx->resource->get('id') ? $this->modx->makeUrl($this->modx->resource->get('id'),'','&cmp_action=empty') : '',
...
function deleteCompareProduct(){
    ...
    $this->modx->sendRedirect($this->modx->makeUrl($this->modx->resource->get('id')),0,'REDIRECT_HEADER');
    exit;
}
