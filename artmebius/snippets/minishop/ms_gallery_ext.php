...
$additional_placeholders = $default_properties = array();
if (isset($this) && $this instanceof modSnippet && $this->get('properties')) {
    $default_properties = $this->get('properties');
}
elseif ($snippet = $modx->getObject('modSnippet', array('name' => 'msGalleryExt'))) {
    $default_properties = $snippet->get('properties');
}

if (!empty($default_properties)) {
    foreach ($scriptProperties as $k => $v) {
        if (!isset($default_properties[$k])) {
            $additional_placeholders[$k] = $v;
        }
    }
}
...
$output = $pdoFetch->getChunk($tpl, array_merge($additional_placeholders, [
    'files' => $files,
