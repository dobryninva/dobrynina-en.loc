...
(function (window, document, $, msOptionsPriceConfig) {
    ...
    msOptionsPrice.setup = function () {
        ...
        msOptionsPrice.Product.prev_iid = 0;
    ...
    msOptionsPrice.initialize = function () {
        ...
        // slick
        // $('.prdt_imgs_full').on('beforeChange', function(event, slick, currentSlide, nextSlide){
        //   msOptionsPrice.Product.prev_iid = (msOptionsPrice.Product.prev_iid == 0 || msOptionsPrice.Product.prev_iid == $(slick.$slides[nextSlide]).find('.lightbox').data('iid')) ? $(slick.$slides[nextSlide]).find('.lightbox').data('iid') : $(slick.$slides[currentSlide]).find('.lightbox').data('iid');
        // });
        // $('.prdt_imgs_full').on('afterChange', function(event, slick, currentSlide){
        //   let curr_rid = $(slick.$slides[currentSlide]).find('.lightbox').data('rid'),
        //       curr_iid = $(slick.$slides[currentSlide]).find('.lightbox').data('iid');

        //   if (curr_iid != msOptionsPrice.Product.prev_iid) {
        //     let form = $(msOptionsPrice.Product.cost + msOptionsPrice.Product.prefix + curr_rid).closest(msOptionsPrice.Product.form);
        //     if (form.length) {
        //       msOptionsPrice.Product.action('modification/get', form, {id: curr_rid, iid: curr_iid});
        //       msOptionsPrice.Product.prev_iid = curr_iid;
        //     }
        //   }
        // });
    ...
    msOptionsPrice.Product = {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.success && response.data) {
                        ...
                        /* set image  */
                        // $('.prdt_imgs_full').slick('slickUnfilter');
        ...
        processOptions: function (form) {
            ...
            $related.filter(function () {
                ...
                currConstraints.filter(function (key) {
                    if (options.hasOwnProperty(key)) {
                      if (!msOptionsPrice.Tools.inArray($form.find('[name="options['+key+']"]')[0], $disabled)) {
            ...
            // скрываем тп полностью, если все опции неактивны
            $('.attr-parent').each(function(index, el) {
              let show_attr = 0;
              $(this).find(selector).each(function(index, el) {
    ...
    msOptionsPrice.Tools = {
        ...
        setOptionValue: function (key, value, rid) {
            ...
            if (inputs) {
                inputs.each(function () {
                    // if($(this).is(':checked')){
                    //     $('#migxTorgDropdown input').val(value);
                    // }
        ...
        setGalleryImage: function (rid, iid) {
            ...
            // $('.prdt_imgs_full').slick('slickUnfilter');
            // $('.prdt_imgs_full').slick('slickFilter',function(filter){
            //   return $(this).find('[data-iid="' + iid + '"]').length;
            // });

            // let slide_curr = $('.prdt_imgs_full').slick('slickCurrentSlide'),
            //     slide_next = $('.prdt_imgs_full [data-iid="' + iid + '"]').parents('.slick-slide').data('slick-index');

            // msOptionsPrice.Product.prev_iid = iid;
            // if (slide_curr != slide_next) $('.prdt_imgs_full').slick('slickGoTo', slide_next);
