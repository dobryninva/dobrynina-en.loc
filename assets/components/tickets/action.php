...
if (!empty($_REQUEST['thread']) && !empty($_REQUEST['resource'])) {
    $resourceThread = $modx->getObject('modResource', $_REQUEST['resource']);

    if (!$thread = $modx->getObject('TicketThread', array('name' => $_REQUEST['thread']))) {
      $thread = $modx->newObject('TicketThread');
      $thread->fromArray(array(
        'name' => $_REQUEST['thread'],
        // 'name' => 'resource-'.$modx->resource->get('id'),
        'resource' => $resourceThread->id,
        'createdby' => $modx->user->id,
        'createdon' => date('Y-m-d H:i:s'),
        'subscribers' => array($resourceThread->get('createdby')),
        'properties' => $_SESSION['TicketCommentsProperties'],
      ));
      $thread->save();
    }
